...
class DeepQLearningAgent(Agent):
    ...
    def train_agent(self, batch_size=32, num_games=1, reward_clip=False):
        ...
        next_model_outputs = self._get_model_outputs(next_s, current_model).detach().numpy()
        ...
        model_output = self._get_model_outputs(self._normalize_board(s))
        loss = self._criterion(model_output.float(), target.float())
